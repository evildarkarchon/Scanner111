...
public class PapyrusLogAnalysis
{
    ...
    public string FormattedMessage
    {
        get
        {
            ...
            return string.Join(Environment.NewLine, $"NUMBER OF DUMPS    : {DumpCount}",
                $"NUMBER OF STACKS   : {StackCount}", $"DUMPS/STACKS RATIO : {Math.Round(DumpStackRatio, 3)}",
                $"NUMBER OF WARNINGS : {WarningCount}", $"NUMBER OF ERRORS   : {ErrorCount}");
        }
    }
}