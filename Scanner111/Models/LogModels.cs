...
public class ParsedCrashLog
{
    ...
    public Dictionary<string, string> LoadedPlugins { get; } = new(StringComparer.OrdinalIgnoreCase);
    public List<string> CallStack { get; } = [];
    public List<string> MainErrorSegment { get; } = [];
    public Dictionary<string, List<string>> OtherSegments { get; } = new(); // For other named segments
}