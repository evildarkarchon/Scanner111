...
public class CrashLogScanService : ICrashLogScanService
{
    ...
    private Regex? _pluginSearchRegex;
    ...
    public async Task<CrashLogProcessResult> ProcessSingleCrashLogAsync(string logFileName)
    {
        ...
        try
        {
            ...
            {
                autoscanReport.Add(_mainFilesResult);
            }

            if (!string.IsNullOrEmpty(_gameFilesResult))
            {
                autoscanReport.Add(_gameFilesResult);
            }
            ...
            if (!suspectFound)
            {
                autoscanReport.Add("# ℹ️ No suspect patterns found in main error or call stack # \n-----\n");
            ...
            {
                autoscanReport.Add($"## Found {crashlogPlugins.Count} plugins\n");
    ...
    public Task<CrashLogSegments> FindSegmentsAsync(List<string> crashData, string crashgenName)
    {
        return Task.Run(() =>
        {
            ...
            foreach (var line in crashData)
            {
                ...
                if (inCrashgen) crashgenSegment.Add(line);
                else if (inSystem) systemSegment.Add(line);
                else if (inCallStack) callStackSegment.Add(line);
                else if (inModules)
                {
                    allModulesSegment.Add(line);
                    if (line.Contains("f4se") || line.Contains("skse"))
                    {
                        xseModulesSegment.Add(line);
                    }
                }
                else if (inPlugins) pluginsSegment.Add(line);
            }
        ...
        Dictionary<string, bool> matchStatus)
    {
        ...
        if (!signal.Contains('|'))
        {
            ...
            if (callStack.Contains(signal, StringComparison.OrdinalIgnoreCase))
            {
        ...
        else if (signalModifier.Equals(callStackNegative, StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (callStack.Contains(signalString, StringComparison.OrdinalIgnoreCase))
            {
                return true; // Signal to skip this error
            }
        }
    ...
    private bool IsSuspectStackMatch(Dictionary<string, bool> matchStatus)
    {
        ...
        if (matchStatus["has_required_item"] && !matchStatus["error_req_found"])
        {
            return false;
        }
    ...
    {
        if (_yamlData == null || string.IsNullOrEmpty(crashlogMainError))
        {
            return false;
        }
    ...
    {
        if (callStackSegment.Count == 0 || pluginsMap.Count == 0 || !_databaseService.DatabaseExists())
        {
            return;
        }
        ...
        if (!formIdsFound)
        {
            autoscanReport.Add("❓ No FormIDs from call stack matched to plugins in database\n");
        }
    }
