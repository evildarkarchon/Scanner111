...
/// </summary>
public partial class CheckXsePluginsService : ICheckXsePluginsService
{
    ...
    private async Task CheckAddressLibraryAsync(string gameDir, string xseName, StringBuilder results)
    {
        ...
        var expectedAddrLibFile = $"version-1-{expectedVersion.Replace('.', '-')}.bin";
        var expectedAddrLibPath = Path.Combine(gameDir, "Data", $"{xseName}", "Plugins", expectedAddrLibFile);

        // In test mode or real mode, check if the expected file exists
        var addrLibExists = _testMode || File.Exists(expectedAddrLibPath);
        ...
        if (!addrLibExists)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    foreach (var file in versionFiles) results.AppendLine($"     - {Path.GetFileName(file)}");
    ...
    private async Task CheckXsePluginsVersionsAsync(string gameDir, string xseName, StringBuilder results)
    {
        ...
        {
            if (!plugin.TryGetValue("name", out var pluginName) || string.IsNullOrEmpty(pluginName)) continue;

            plugin.TryGetValue("min_version", out var minVersion);

            // Check if plugin exists
            var pluginPath = Path.Combine(pluginsPath, pluginName);
            var pluginExists = _testMode || File.Exists(pluginPath);
    ...
    /// </summary>
    /// <param name="v1">The first version string to compare.</param>
    /// <param name="v2">The second version string to compare.</param>
    /// <returns>
    ///     Returns 1 if the first version is greater, -1 if the second version is greater, and 0 if both versions are equal.
    /// </returns>
    private int CompareVersions(string v1, string v2)
    {
        ...
        {
            var v1Val = i < v1Parts.Length ? v1Parts[i] : 0;
            var v2Val = i < v2Parts.Length ? v2Parts[i] : 0;
    ...
    [GeneratedRegex(@"(?:Version:|v)[.\s]*(\d+\.\d+\.\d+(?:\.\d+)?)")]
