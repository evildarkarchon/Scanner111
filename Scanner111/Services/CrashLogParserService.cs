...
namespace Scanner111.Services
{
    ...
    /// </summary>
    public class CrashLogParserService
    {
        // Regex for parsing initial version lines
        private static readonly Regex FalloutVersionRegex = new Regex(@"^Fallout 4 v(\d+\.\d+\.\d+.*)", RegexOptions.IgnoreCase);
        private static readonly Regex BuffoutVersionRegex = new Regex(@"^Buffout 4 v(\d+\.\d+\.\d+.*)", RegexOptions.IgnoreCase);
        private static readonly Regex GenericCrashGenRegex = new Regex(@"^([a-zA-Z0-9\s]+?) v(\d+\.\d+\.\d+.*)", RegexOptions.IgnoreCase); // Broader match for other crash gens
        ...
        private static readonly Regex PluginEntryRegex = new Regex(
        ...
        public async Task<ParsedCrashLog> ParseCrashLogContentAsync(string logFilePath)
        {
            ...
            {
                return new ParsedCrashLog(logFilePath, new List<string>()); // Return empty if file not found
            }
            ...
            {
                string currentLine = lines[lineIndex]; // Keep original line for content
                ...
                if (string.IsNullOrWhiteSpace(trimmedLine))
                {
                    ...
                    {
                        if (currentSegmentKey == ProbableCallStackHeader) parsedLog.CallStack.Add(currentLine);
                        else if (currentSegmentKey == PluginsHeader) { /* PLUGINS section usually doesn't have empty lines as valid entries */ }
                        else if (parsedLog.OtherSegments.TryGetValue(currentSegmentKey, out var segmentList)) segmentList.Add(currentLine);
                    }
                ...
                if (trimmedLine.StartsWith(SystemSpecsHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = SystemSpecsHeader;
                else if (trimmedLine.StartsWith(ProbableCallStackHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = ProbableCallStackHeader;
                else if (trimmedLine.StartsWith(RegistersHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = RegistersHeader;
                else if (trimmedLine.StartsWith(StackHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = StackHeader; // This is the detailed one
                else if (trimmedLine.StartsWith(ModulesHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = ModulesHeader;
                else if (trimmedLine.StartsWith(F4SEPluginsHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = F4SEPluginsHeader;
                else if (trimmedLine.StartsWith(PluginsHeader, StringComparison.OrdinalIgnoreCase)) currentSegmentKey = PluginsHeader;
                    ...
                    else // Belongs to an active segment
                    {
                        switch (currentSegmentKey)
                        {
                            ...
                            case PluginsHeader:
                                ...
                                {
                                    string pluginName = pluginMatch.Groups[2].Value;
                                    // Identifier can be index or version, or just "N/A" if neither
                                    string identifier = pluginMatch.Groups[1].Success ? pluginMatch.Groups[1].Value.Trim() :
                            ...
                            default: // For SystemSpecs, Registers, Stack, Modules, F4SEPlugins
                                ...
                                {
                                    parsedLog.OtherSegments[currentSegmentKey] = new List<string>();
                                }
                ...
                if (currentSegmentKey != ProbableCallStackHeader && currentSegmentKey != PluginsHeader && !parsedLog.OtherSegments.ContainsKey(currentSegmentKey))
                {
                    parsedLog.OtherSegments[currentSegmentKey] = new List<string>();
                }
            }
