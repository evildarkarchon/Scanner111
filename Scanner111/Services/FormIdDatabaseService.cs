    ...
    public class FormIdDatabaseService : IDisposable
    {
        ...
        private void InitializeDatabase()
        {
            ...
            try
            {
                ...
                _dbConnection = new SQLiteConnection($"Data Source={_appSettings.FormIdDatabasePath};Version=3;Read Only=True;");
                _dbConnection.Open();
        ...
        public string? GetEntry(string formId, string plugin)
        {
            ...
            try
            {
                ...
                cmd.CommandText = $"SELECT entry FROM {_appSettings.GameName} WHERE formid=@formid AND plugin=@plugin COLLATE nocase";
            ...
            string formIdColumn = "FormID", string pluginColumn = "Plugin", bool hasHeaderRow = true)
        {
            ...
            try
            {
                ...
                using var command = connection.CreateCommand();
                command.CommandText = $"INSERT OR REPLACE INTO {gameName} (formid, plugin, entry) VALUES (@formid, @plugin, @entry)";
                ...
                if (hasHeaderRow)
                {
                    ...
                    int formIdColIndex = Array.FindIndex(headers, h => h.Equals(formIdColumn, StringComparison.OrdinalIgnoreCase));
                    int pluginColIndex = Array.FindIndex(headers, h => h.Equals(pluginColumn, StringComparison.OrdinalIgnoreCase));
                    int edidColIndex = Array.FindIndex(headers, h => h.Equals("EditorID", StringComparison.OrdinalIgnoreCase));
                    int nameColIndex = Array.FindIndex(headers, h => h.Equals("Name", StringComparison.OrdinalIgnoreCase));
