...
public class YamlSettingsCache : IYamlSettingsCache
{
    ...
    {
        if (_pathCache.TryGetValue(yamlStore, out var path))
        {
            return path;
        }
        
        string yamlPath;
        string dataPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "CLASSIC Data/");
    ...
    public Dictionary<string, object> LoadYaml(string yamlPath)
    {
        ...
        bool isStatic = YamlConstants.StaticYamlStores.Any(store => 
            yamlPath == GetPathForStore(store));
        
        // For static files, just load once
        if (isStatic && _cache.TryGetValue(yamlPath, out var cachedData))
        {
            return cachedData;
        }
        ...
        if (!isStatic)
        {
            DateTime lastModTime = File.GetLastWriteTime(yamlPath);
