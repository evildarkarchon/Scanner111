...
/// </summary>
public partial class ScanWryeCheckService : IScanWryeCheckService
{
    ...
    {
        if (_testMode) return "test_report.txt";
        ...
        await Task.Run(() =>
        {
            try
            {
                ...
                foreach (var file in reportFiles)
                {
                    ...
                    if (fileInfo.LastWriteTime <= latestDate) continue;
    ...
    private async Task AnalyzeWildEditsAsync(string reportContent, List<ReportIssue> issues)
    {
        await Task.Run(() =>
        {
            var wildEditPattern = GetWildEditPattern();
    ...
    private async Task AnalyzeFormVersionsAsync(string reportContent, List<ReportIssue> issues)
    {
        await Task.Run(() =>
        {
            var formVersionPattern = GetFormVersionPattern();
    ...
    [GeneratedRegex(@"ITM, Identical to Master\s*\((\d+)\):\s*(.+?)(?:\r?\n|$)")]
    private static partial Regex GetItmPattern();

    [GeneratedRegex(@"UDR, Undeleted References\s*\((\d+)\):\s*(.+?)(?:\r?\n|$)")]
    private static partial Regex GetUdrPattern();

    [GeneratedRegex(@"Wild Edits\s*\((\d+)\):\s*(.+?)(?:\r?\n|$)")]
    private static partial Regex GetWildEditPattern();

    [GeneratedRegex(@"FormID Versions\s*\((\d+)\):\s*(.+?)(?:\r?\n|$)")]
    private static partial Regex GetFormVersionPattern();
