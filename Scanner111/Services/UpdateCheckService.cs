...
namespace Scanner111.Services
{
    ...
    public class UpdateCheckService : IUpdateCheckService
    {
        ...
        public Version? TryParseVersion(string? versionStr)
        {
            ...
            string potentialVersionPart = versionStr.Split(' ').LastOrDefault() ?? versionStr;
        ...
        private async Task<Version?> GetGithubLatestPrereleaseVersionFromListAsync(HttpClient client, string owner, string repo)
        {
                ...
                foreach (var releaseData in releasesArray)
                {
                        ...
                        prereleaseVal.ValueKind == JsonValueKind.True)
                    {
                        if (releaseData.TryGetProperty("name", out var nameVal) &&
                            nameVal.ValueKind == JsonValueKind.String)
                        {
                            string? releaseName = nameVal.GetString();
                            if (!string.IsNullOrEmpty(releaseName))
                            {
                                Version? parsedVersion = TryParseVersion(releaseName);
            ...
            {
                _logger?.LogError($"Error fetching releases list from {url}: {ex.Message}");
            }
        ...
        private async Task<Dictionary<string, object?>?> GetLatestAndTopReleaseDetailsAsync(HttpClient client, string owner, string repo)
        {
            ...
            try
            {
                ...
                {
                    _logger?.LogWarning($"No releases found or unexpected format from {allReleasesUrl}");
                }

                // Check if they're the same release
                var latestEndpointRelease = results["latest_endpoint_release"] as Dictionary<string, object?>;
                var topOfListRelease = results["top_of_list_release"] as Dictionary<string, object?>;

                if (latestEndpointRelease != null && topOfListRelease != null &&
            ...
            {
                _logger?.LogError($"GitHub API ClientError for {owner}/{repo}: {ex.Message}");
            ...
            {
                _logger?.LogError($"Unexpected error fetching release details for {owner}/{repo}: {ex.Message}");
                return null;
            }
        }
        ...
        {
            // Constants
            string nexusModUrl = "https://www.nexusmods.com/fallout4/mods/56255";
            string versionPropertyName = "twitter:label1";
            string versionPropertyValue = "Version";
            string versionDataProperty = "twitter:data1"; // This line was identified with CS0219
            ...
            try
            {
                ...
                string versionStr = versionDataTag.GetAttributeValue("content", string.Empty);
                if (!string.IsNullOrEmpty(versionStr))
                {
                    Version? parsedVersion = TryParseVersion(versionStr);
                    if (parsedVersion != null)
                    {
                        _logger?.LogDebug($"Successfully parsed Nexus version: {parsedVersion}");
                        return parsedVersion;
                    }
                    else
                    {
                        _logger?.LogDebug($"Failed to parse version string: '{versionStr}'");
                    }
                }
                else
                {
            ...
            {
                _logger?.LogError($"Network error while fetching Nexus version: {ex.Message}");
            }
            catch (Exception ex)
            {
                _logger?.LogError($"Unexpected error parsing Nexus version: {ex.Message}");
            }
        ...
        public async Task<bool> IsLatestVersionAsync(bool quiet = false, bool guiRequest = true)
        {
            ...
            string repoOwner = "evildarkarchon"; // Update with your actual repo owner
            string repoName = "Scanner111"; // Update with your actual repo name
            ...
            string? updateSource = GetSetting<string>(YAML.Settings, "Update Source") ?? "Both";
            ...
            {
                string[] parts = localVersionStr.Split(' ');
                if (parts.Length > 0)
                {
                    parsedLocalVersionStr = parts.Last();
                }
            }

            Version? localVersion = parsedLocalVersionStr != null ? TryParseVersion(parsedLocalVersionStr) : null;
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (useNexus)
                    {
                        ...
                        {
                            _logger?.LogInformation($"Determined Nexus version: {nexusVersionToCompare}");
                        }
                    }

                    bool nexusSourceFailed = useNexus && nexusVersionToCompare == null;
                    bool githubSourceFailed = useGithub && githubVersionToCompare == null;
            ...
            {
                _logger?.LogError($"Update check failed during version fetching: {ex.Message}");
                if (!quiet)
                {
                    Console.WriteLine(ex.Message);

                    string? unableMsg = GetSetting<string>(YAML.Main, "Scanner111_Interface.update_unable");
                    if (!string.IsNullOrEmpty(unableMsg))
                    {
            ...
            {
                _logger?.LogError($"Unexpected error during update check: {ex.Message}");
                if (guiRequest)
                {
            ...
            if (localVersion == null)
            {
                ...
                if (githubVersionToCompare != null || nexusVersionToCompare != null)
                {
                    isOutdated = true;
                }
            }
            ...
            if (!quiet)
            {
                ...
                if (useGithub && githubVersionToCompare != null)
                {
                ...
                if (useNexus && nexusVersionToCompare != null)
                {
