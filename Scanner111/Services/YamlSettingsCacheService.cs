...
public sealed class YamlSettingsCacheService : IYamlSettingsCacheService
{
    ...
    private readonly Dictionary<Yaml, YamlNode> _yamlCache = new();
    ...
    private readonly Dictionary<Yaml, string>
        _yamlFilePaths = new(); // Maps enum to actual file paths
    ...
    private YamlSettingsCacheService()
    {
        try
        {
            ...
            var classicDataDirectory = Path.Combine(_applicationRootPath, _classicDataPath);
            ...
            var gameYamlPath = Path.Combine(_applicationRootPath, _classicDataPath, "databases",
                "CLASSIC Fallout4.yaml");
            var databasesDir = Path.Combine(_applicationRootPath, _classicDataPath, "databases");
    ...
    public T? GetSetting<T>(Yaml yamlType, string keyPath, T? defaultValue = default)
    {
        ...
        var value = GetValueFromPath(yamlData, keys);
        ...
        try
        {
            ...
            if (typeof(T) == typeof(bool))
            {
                ...
                if (int.TryParse(Convert.ToString(value), out var intVal)) return (T)(object)(intVal != 0);
            }
    ...
    {
        var filePath = GetPathForStore(yamlType);
        var yamlData = GetOrLoadYamlData(yamlType);
    ...
    private void InitializeStaticCaches()
    {
        try
        {
            ...
            foreach (var yamlType in StaticYamlStores)
                try
                {
                    var filePath = GetPathForStore(yamlType);
    ...
    public string GetPathForStore(Yaml yamlType)
    {
        return yamlType switch
        {
            ...
            Yaml.GameLocal => Path.Combine(_applicationRootPath, _classicDataPath,
    ...
    {
        if (_yamlCache.TryGetValue(yamlStore, out var cachedNode)) return cachedNode;
