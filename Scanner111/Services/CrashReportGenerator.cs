...
public class CrashReportGenerator
{
    ...
    public string GenerateReport(string crashLogPath, List<LogIssue> issues)
    {
        ...
        if (criticalIssues.Count != 0)
        {
            ...
            reportBuilder.AppendLine("==================================================================");
            foreach (var issue in criticalIssues) AppendIssue(reportBuilder, issue);
        ...
        if (errorIssues.Count != 0)
        {
            ...
            reportBuilder.AppendLine("==================================================================");
            foreach (var issue in errorIssues) AppendIssue(reportBuilder, issue);
        ...
        if (warningIssues.Count != 0)
        {
            ...
            reportBuilder.AppendLine("==================================================================");
            foreach (var issue in warningIssues) AppendIssue(reportBuilder, issue);
        ...
        if (infoIssues.Count != 0)
        {
            ...
            reportBuilder.AppendLine("==================================================================");
            foreach (var issue in infoIssues) AppendIssue(reportBuilder, issue);
    ...
    private void AppendIssue(StringBuilder reportBuilder, LogIssue issue)
    {
        ...
        if (!string.IsNullOrEmpty(issue.Details)) reportBuilder.AppendLine($"Details: {issue.Details}");
    ...
    private string GetRandomHint()
    {
        try
        {
            ...
            var index = random.Next(hints.Count);
            return hints[index];
        }
        catch (Exception)
        {
    ...
    public void SaveReportToFile(string reportContent, string crashLogPath, string outputDirectory)
    {
        try
        {
            ...
            var baseFilename = Path.GetFileNameWithoutExtension(crashLogPath);
            var reportFilename = $"{baseFilename}_report_{DateTime.Now:yyyyMMdd_HHmmss}.txt";
    ...
    public void MoveUnsolvedLog(string crashLogPath, string unsolvedDirectory)
    {
        try
        {
            ...
            if (!Directory.Exists(unsolvedDirectory)) Directory.CreateDirectory(unsolvedDirectory);

            // Generate a filename for the unsolved log
            var filename = Path.GetFileName(crashLogPath);
            var unsolvedFilePath = Path.Combine(unsolvedDirectory, filename);
