...
namespace Scanner111.Services
{
    ...
    public class GameDirectoryService : IGameDirectoryService
    {
        ...
        public async Task<string> FindGamePathAsync()
        {
            ...
            string existingGamePath = GetSetting<string>(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Root_Folder_Game");
            if (!string.IsNullOrEmpty(existingGamePath) && Directory.Exists(existingGamePath))
            {
                ...
                string exeName = $"{_appSettings.GameName}{GetVrSuffix()}.exe";
            ...
            string registryPath = await FindGamePathFromRegistryAsync();
            if (!string.IsNullOrEmpty(registryPath))
            {
                if (ValidateGamePath(registryPath, out string exePath))
                {
            ...
            string xseLogPath = await FindGamePathFromXseLogAsync();
            if (!string.IsNullOrEmpty(xseLogPath))
            {
                if (ValidateGamePath(xseLogPath, out string exePath))
                {
        ...
        public async Task<string> FindDocsPathAsync()
        {
            ...
            // Check if path is already configured
            string existingDocsPath = GetSetting<string>(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Root_Folder_Docs");
            ...
            string docsPath = await FindDocsPathFromWindowsProfileAsync();
        ...
        public async Task<bool> SetGamePathManuallyAsync(string path)
        {
            ...
            if (ValidateGamePath(path, out string exePath))
            {
        ...
        private async Task<string> FindGamePathFromXseLogAsync()
        {
            try
            {
                ...
                string xseAcronym = GetSetting<string>(YAML.Game, $"Game{GetVrSuffix()}_Info.XSE_Acronym");
                string xseLogFile = GetSetting<string>(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Docs_File_XSE");
                ...
                string[] logLines = await File.ReadAllLinesAsync(xseLogFile);
                foreach (string line in logLines)
                {
                    if (line.StartsWith("plugin directory", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (parts.Length == 2)
                        {
                            string pluginDirPath = parts[1].Trim();
                            // Remove the plugin part of the path to get the game directory
                            string gamePath = pluginDirPath.Replace($@"\Data\{xseAcronym}\Plugins", "");
        ...
        private async Task<string> FindDocsPathFromWindowsProfileAsync()
        {
            try
            {
                ...
                string docsName = GetSetting<string>(YAML.Game, $"Game{GetVrSuffix()}_Info.Main_Docs_Name");
                if (string.IsNullOrEmpty(docsName))
                {
                    docsName = _appSettings.GameName; // Default to game name
                }
                           ...
                           @"Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders"))
                {
                    if (key != null)
                    {
                        documentsPath = key.GetValue("Personal") as string;
                    }
                }
                ...
                {
                    string fullDocsPath = Path.Combine(documentsPath, "My Games", docsName);
                    if (Directory.Exists(fullDocsPath))
                    {
                        return fullDocsPath;
                    }
                }
        ...
        private void SaveGamePath(string path)
        {
            ...
            try
            {
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Root_Folder_Game", path);
            }
            catch (Exception ex)
            {
        ...
        private void SaveDocsPath(string path)
        {
            ...
            try
            {
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Root_Folder_Docs", path);
            }
            catch (Exception ex)
            {
        ...
        private void GenerateGamePaths(string gamePath)
        {
            ...
            try
            {
                string xseAcronymBase = GetSetting<string>(YAML.Game, "Game_Info.XSE_Acronym");
                if (string.IsNullOrEmpty(xseAcronymBase))
                {
                    xseAcronymBase = "F4SE"; // Default to F4SE
                }

                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_Folder_Data",
                    Path.Combine(gamePath, "Data"));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_Folder_Scripts",
                    Path.Combine(gamePath, "Data", "Scripts"));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_Folder_Plugins",
                    Path.Combine(gamePath, "Data", xseAcronymBase, "Plugins"));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_File_SteamINI",
                    Path.Combine(gamePath, "steam_api.ini"));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_File_EXE",
                    Path.Combine(gamePath, $"{_appSettings.GameName}{GetVrSuffix()}.exe"));

                // Set Address Library path based on game (Fallout4 vs others)
                if (_appSettings.GameName.Contains("Fallout4", StringComparison.OrdinalIgnoreCase))
                {
                    if (GetVrSuffix() == "VR")
                    {
                        _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_File_AddressLib",
                            Path.Combine(gamePath, "Data", xseAcronymBase, "plugins", "version-1-2-72-0.csv"));
                    }
                    else
                    {
                        string gameVersion = GetSetting<string>(YAML.Game, "game_version") ?? "1-10-163-0";
                        _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Game_File_AddressLib",
        ...
        private void GenerateDocsPaths(string docsPath)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(xseAcronym))
                {
                    xseAcronym = "F4SE"; // Default
                }

                if (string.IsNullOrEmpty(xseAcronymBase))
                {
                    xseAcronymBase = "F4SE"; // Default
                }

                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Docs_Folder_XSE",
                    Path.Combine(docsPath, xseAcronymBase));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Docs_File_PapyrusLog",
                    Path.Combine(docsPath, "Logs", "Script", "Papyrus.0.log"));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Docs_File_WryeBashPC",
                    Path.Combine(docsPath, "ModChecker.html"));
                _yamlSettingsCache.SetSetting(YAML.Game_Local, $"Game{GetVrSuffix()}_Info.Docs_File_XSE",
        ...
        {
            if (!_testMode && !Directory.Exists(directoryPath))
            {
