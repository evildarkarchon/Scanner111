...
public class FcxModeHandlerTests : IDisposable
{
    ...
    public FcxModeHandlerTests()
    {
        ...
        CreateHandler(fcxMode: false).ResetFcxChecks();
    }
    ...
    {
        // Arrange
        var handler = CreateHandler(fcxMode: true);
    ...
    {
        // Arrange
        var handler = CreateHandler(fcxMode: false);
    ...
    {
        // Arrange
        var handler1 = CreateHandler(fcxMode: true);
        var handler2 = CreateHandler(fcxMode: true);
    ...
    public async Task CheckFcxModeAsync_ShouldBeThreadSafe_WithConcurrentCalls()
    {
        ...
        var handlers = Enumerable.Range(0, concurrentTasks)
            .Select(_ => CreateHandler(fcxMode: true))
            .ToList();
    ...
    public async Task CheckFcxModeAsync_ShouldHandleCancellation()
    {
        ...
        await Assert.ThrowsAsync<TaskCanceledException>(
            () => handler.CheckFcxModeAsync(cts.Token));
    }
    ...
    {
        // Arrange & Act
        var handler = CreateHandler(fcxMode: fcxMode);
