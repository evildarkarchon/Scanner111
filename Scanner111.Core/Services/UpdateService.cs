...
public class UpdateService : IUpdateService
{
    ...
    public async Task<bool> IsLatestVersionAsync(bool quiet = false, CancellationToken cancellationToken = default)
    {
        ...
        if (!result.CheckSuccessful)
        {
            if (!quiet)
            {
                _messageHandler.ShowError($"Update check failed: {result.ErrorMessage}");
            }
            return false;
        }
    ...
    public async Task<UpdateCheckResult> GetUpdateInfoAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (useGitHub)
            {
                ...
                {
                    result.LatestGitHubVersion = await GetLatestGitHubVersionAsync(cancellationToken).ConfigureAwait(false);
                }, cancellationToken));
            }
            ...
            if (useNexus)
            {
                tasks.Add(Task.Run(async () =>
                {
                    result.LatestNexusVersion = await GetLatestNexusVersionAsync(cancellationToken).ConfigureAwait(false);
                }, cancellationToken));
            }
            ...
            result.IsUpdateAvailable = IsUpdateAvailable(result.CurrentVersion, result.LatestGitHubVersion, result.LatestNexusVersion);
    ...
    private async Task<Version?> GetLatestGitHubVersionAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (root.TryGetProperty("name", out var nameElement))
            {
                ...
                if (version != null)
                {
                    _logger.LogInformation($"Found GitHub version: {version}");
                }
                return version;
            }
    ...
    private async Task<Version?> GetLatestNexusVersionAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (versionMatch.Success)
            {
                ...
                if (version != null)
                {
                    _logger.LogInformation($"Found Nexus version: {version}");
                }
                return version;
            }
    ...
    private static Version? TryParseVersion(string? versionString)
    {
        ...
        try
        {
            ...
            {
                potentialVersionPart = potentialVersionPart[1..];
            }
    ...
    private static bool IsUpdateAvailable(Version? currentVersion, Version? gitHubVersion, Version? nexusVersion)
    {
        ...
        if (gitHubVersion != null && currentVersion < gitHubVersion)
        {
            return true;
        }

        if (nexusVersion != null && currentVersion < nexusVersion)
        {
            return true;
        }
