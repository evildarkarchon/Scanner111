    ...
    public class GameScannerService : IGameScannerService
    {
        ...
        public async Task<GameScanResult> ScanGameAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                // Process results
                foreach (var (type, output) in results)
                {
                    ProcessScanResult(result, type, output);
                }
                ...
                if (result.HasIssues)
                    _messageHandler.ShowWarning($"Game scan completed in {stopwatch.Elapsed.TotalSeconds:F1} seconds with issues found");
                else
        ...
        private void AnalyzeResults(GameScanResult result)
        {
            ...
            if (allOutput.Contains("❌"))
            {
                ...
                foreach (var line in lines.Where(l => l.Contains("❌")))
                {
                    ...
                    {
                        result.CriticalIssues.Add(cleanLine);
                    }
                }
            }
            ...
            if (allOutput.Contains("⚠️"))
            {
                ...
                foreach (var line in lines.Where(l => l.Contains("⚠️")))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(cleanLine) && cleanLine.Length > 5)
                    {
                        result.Warnings.Add(cleanLine);
                    }
                }
            }
            
            // Log summary
            _logger.LogInformation($"Scan complete: {result.CriticalIssues.Count} critical issues, {result.Warnings.Count} warnings");
        }
    }
}