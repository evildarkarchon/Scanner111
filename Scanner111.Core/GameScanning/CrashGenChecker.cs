...
public class CrashGenChecker : ICrashGenChecker
{
    ...
    {
        if (string.IsNullOrEmpty(_pluginsPath) || !Directory.Exists(_pluginsPath)) return null;
        ...
        if (File.Exists(crashGenToml)) return crashGenToml;
    ...
    {
        if (string.IsNullOrEmpty(_pluginsPath) || !Directory.Exists(_pluginsPath)) return;
    ...
    {
        if (string.IsNullOrEmpty(_configFile) || !File.Exists(_configFile)) return;
