    ...
    public class XsePluginValidator : IXsePluginValidator
    {
        ...
        public async Task<string> ValidateAsync()
        {
            await Task.Run(() =>
            {
                ...
                {
                    _messageList.AddRange(FormatWrongAddressLibMessage(correctVersions.FirstOrDefault() ?? new AddressLibVersionInfo()));
                }
                else
                {
                    _messageList.AddRange(FormatAddressLibNotFoundMessage(correctVersions.FirstOrDefault() ?? new AddressLibVersionInfo()));
                }
        ...
        private void CheckXsePluginCompatibility(string pluginsPath, GameType gameType)
        {
            ...
            var gameExeDir = Path.GetDirectoryName(_settingsService.LoadSettingsAsync().GetAwaiter().GetResult().GameExecutablePath) ?? "";
        ...
        private (string name, string dll, string url) GetXseInfo(GameType gameType)
        {
            return gameType switch
            {
                ...
                GameType.SkyrimSE => ("SKSE64", "skse64_1_6_1170.dll", "https://www.nexusmods.com/skyrimspecialedition/mods/30379"),
                GameType.SkyrimVR => ("SKSEVR", "sksevr_2_0_12.dll", "https://www.nexusmods.com/skyrimspecialedition/mods/30457"),
        ...
        private List<(string plugin1, string plugin2, string message)> GetConflictsForGame(GameType gameType)
        {
            ...
            if (gameType == GameType.SkyrimSE || gameType == GameType.SkyrimVR || gameType == GameType.Skyrim)
            {
                conflicts.AddRange(new[]
                {
                    ...
                    ("enginefixes.dll", "ssefixesng.dll", "Engine Fixes and SSE Fixes NG provide overlapping functionality"),
