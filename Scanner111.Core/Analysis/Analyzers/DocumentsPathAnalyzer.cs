...
public sealed class DocumentsPathAnalyzer : AnalyzerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(iniCheck.message))
        {
            ...
            if (!iniCheck.isValid) hasErrors = true;
        }
        ...
        if (!hasWarnings && !hasErrors) report.AppendLine("✅ Documents folder configuration is optimal");
        ...
        if (!string.IsNullOrEmpty(docsPath)) context.SetSharedData("DocumentsPath", docsPath);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(docsPath)) return (false, "⚠️ Documents path not configured");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(docsPath)) return (false, string.Empty);
            ...
            foreach (var iniFile in iniFiles)
            {
                ...
                if (!File.Exists(fullPath))
                {
                    ...
                    if (iniFile != "Fallout4Custom.ini") missingFiles.Add(iniFile);
                }
                else
                {
                    // Check if file is read-only
                    var fileInfo = new FileInfo(fullPath);
                    if (fileInfo.IsReadOnly) readOnlyFiles.Add(iniFile);
                }
            }

            var messages = new List<string>();

            if (missingFiles.Count > 0) messages.Add($"❌ Missing INI files: {string.Join(", ", missingFiles)}");

            if (readOnlyFiles.Count > 0) messages.Add($"⚠️ Read-only INI files: {string.Join(", ", readOnlyFiles)}");

            if (messages.Count > 0) return (missingFiles.Count == 0, string.Join("\n", messages));
