...
public sealed class FcxModeAnalyzer : AnalyzerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (modSettings.FcxMode != null)
            {
                context.SetSharedData("FcxMode", modSettings.FcxMode.Value);
            }
            ...
            result.AddMetadata("MainFilesChecked", (!string.IsNullOrWhiteSpace(_fcxModeHandler.MainFilesCheck)).ToString());
            result.AddMetadata("GameFilesChecked", (!string.IsNullOrWhiteSpace(_fcxModeHandler.GameFilesCheck)).ToString());
    ...
    public override async Task<bool> CanAnalyzeAsync(AnalysisContext context)
    {
        ...
        if (canAnalyze)
        {
            ...
            try
            {
                var modSettings = await _settingsService.LoadModDetectionSettingsAsync(context, default)
                    .ConfigureAwait(false);
