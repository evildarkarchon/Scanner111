...
public sealed class PathValidationAnalyzer : AnalyzerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var exists = pathInfo.Type == PathType.File
    ...
    private string BuildValidationReport(List<PathValidationResult> results)
    {
        ...
        if (validPaths.Any())
        {
            ...
            foreach (var valid in validPaths.Take(5)) sb.AppendLine($"- ✅ **{valid.Name}**: {valid.Message}");

            if (validPaths.Count > 5) sb.AppendLine($"- ... and {validPaths.Count - 5} more valid paths");
        }
