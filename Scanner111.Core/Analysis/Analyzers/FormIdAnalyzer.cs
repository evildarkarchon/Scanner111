...
public sealed partial class FormIdAnalyzer : AnalyzerBase
{
    ...
    private List<string> ExtractFormIds(List<string> callStackSegment)
    {
        ...
        foreach (var line in callStackSegment)
        {
            ...
            if (match.Success)
            {
                ...
                if (!formId.StartsWith("FF", StringComparison.OrdinalIgnoreCase)) formIds.Add($"Form ID: {formId}");
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        reportLines.AppendLine(
            $"These Form IDs were caught by {_crashGenName} and some of them might be related to this crash.");
        reportLines.AppendLine(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Format results
            for (var i = 0; i < lookupTasks.Count; i++)
            {
