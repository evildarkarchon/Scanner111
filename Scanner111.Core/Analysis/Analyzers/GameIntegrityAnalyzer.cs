...
public sealed class GameIntegrityAnalyzer : AnalyzerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!exeCheckResult.isValid) hasWarnings = true;

        // Check installation location
        var locationCheckResult = await CheckInstallationLocationAsync(cancellationToken).ConfigureAwait(false);
        if (!string.IsNullOrWhiteSpace(locationCheckResult.message)) report.AppendLine(locationCheckResult.message);

        if (!locationCheckResult.isValid) hasWarnings = true;

        // Check for missing files
        var filesCheckResult = await CheckRequiredFilesAsync(cancellationToken).ConfigureAwait(false);
        if (!string.IsNullOrWhiteSpace(filesCheckResult.message)) report.AppendLine(filesCheckResult.message);

        if (!filesCheckResult.isValid) hasErrors = true;
        ...
        {
            fragment = ReportFragment.CreateSection("Game Integrity", report.ToString());
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(gamePath)) return (false, "❌ Game executable path not configured");

            if (!File.Exists(gamePath)) return (false, $"❌ Game executable not found at: {gamePath}");
            ...
            if (knownHashes.Contains(hash)) return (true, "✅ Game executable version is up to date");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(gamePath)) return (false, string.Empty);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(gameDir)) return (false, string.Empty);
            ...
            foreach (var dir in requiredFiles)
            {
                ...
                if (!Directory.Exists(fullPath)) missingDirectories.Add(dir);
