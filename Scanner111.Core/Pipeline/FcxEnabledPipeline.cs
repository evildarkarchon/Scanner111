...
public class FcxEnabledPipeline : IScanPipeline
{
    ...
    public async Task<ScanResult> ProcessSingleAsync(string logPath, CancellationToken cancellationToken = default)
    {
        ...
        if (fcxResult != null)
        {
            scanResult = MergeFcxResults(scanResult, fcxResult);
        }
    ...
    private async Task<FcxScanResult?> RunFcxChecksAsync(CrashLog crashLog, CancellationToken cancellationToken)
    {
        try
        {
            _messageHandler.ShowInfo("Running FCX file integrity checks...", MessageTarget.All);
        ...
        catch (Exception ex)
        {
            ...
            _messageHandler.ShowWarning($"FCX check failed: {ex.Message}", MessageTarget.All);
            return null;
        }
    }
    ...
    private async Task<FcxScanResult?> RunFcxChecksForBatchAsync(CancellationToken cancellationToken)
    {
        try
        {
            _messageHandler.ShowInfo("Running FCX file integrity checks for batch...", MessageTarget.All);
        ...
        catch (Exception ex)
        {
            ...
            _messageHandler.ShowWarning($"FCX batch check failed: {ex.Message}", MessageTarget.All);
            return null;
        }
    }
    ...
    private ScanResult MergeFcxResults(ScanResult scanResult, FcxScanResult fcxResult)
    {
        ...
        foreach (var analysis in scanResult.AnalysisResults)
        {
            mergedResult.AnalysisResults.Add(analysis);
        }
        
        // Copy existing error messages
        foreach (var error in scanResult.ErrorMessages)
        {
            mergedResult.ErrorMessages.Add(error);
        }
        ...
        {
            mergedResult.ErrorMessages.Insert(0, "⚠️ FCX detected critical game integrity issues that may affect crash analysis accuracy");
        }
    ...
    private void LogFcxResults(FcxScanResult fcxResult)
    {
        ...
        switch (fcxResult.GameStatus)
        {
            case GameIntegrityStatus.Critical:
                _messageHandler.ShowError("FCX: Critical game integrity issues detected!", MessageTarget.All);
                break;
            case GameIntegrityStatus.Warning:
                _messageHandler.ShowWarning("FCX: Minor game integrity issues detected.", MessageTarget.All);
                break;
            case GameIntegrityStatus.Good:
                _messageHandler.ShowSuccess("FCX: Game integrity checks passed.", MessageTarget.All);
                break;
        }
    }
