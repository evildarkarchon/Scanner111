...
public sealed class FormIdDatabasePool : IFormIdDatabase
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Initializing FormID database pool with max {MaxConnections} connections", _options.MaxConnections);
    ...
    /// <inheritdoc />
    public async Task<string?> GetEntryAsync(string formId, string plugin, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async Task<string?[]> GetEntriesAsync((string formId, string plugin)[] queries, CancellationToken cancellationToken = default)
    {
    ...
    private async Task<string?> QueryDatabaseAsync(string dbPath, string formId, string plugin, CancellationToken cancellationToken)
    {
        ...
        if (_connections.TryGetValue(dbPath, out var existingConnection))
        {
            ...
            if (existingConnection.State == System.Data.ConnectionState.Open)
            {
                ...
                catch
                {
                    ...
                    try { await existingConnection.DisposeAsync().ConfigureAwait(false); } catch { }
                }
            }
        }
