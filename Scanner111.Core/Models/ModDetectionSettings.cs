...
public sealed record ModDetectionSettings
{
    ...
    public static ModDetectionSettings CreateWithXseModules(IEnumerable<string> xseModules)
    {
        ...
        return new ModDetectionSettings
        {
            ...
            HasXCell = modules.Contains("xcell.dll") || modules.Contains("x-cell.dll"),
        ...
        IDictionary<string, object>? metadata = null)
    {
        ...
        var hasXCell = modules.Contains("xcell.dll") || modules.Contains("x-cell.dll");
        var hasBakaScrapHeap = modules.Contains("bakascrapheap.dll") || 
                               modules.Contains("baka_scrapheap.dll");
        
        // Check for old X-Cell version (this would need actual version checking logic)
        var hasOldXCell = false;
        if (hasXCell && metadata != null && metadata.TryGetValue("xcell_version", out var xcellVersion))
        {
            // Simple version check - in real implementation would parse and compare versions
            hasOldXCell = xcellVersion.ToString()?.StartsWith("1.") == true ||
                         xcellVersion.ToString()?.StartsWith("2.") == true;
        }
