...
public static class FcxReportExtensions
{
    public static void AddFcxReportSections(this List<string> report, ScanResult scanResult)
    {
        ...
        if (scanResult.AnalysisResults == null) throw new NullReferenceException();

        // Find FCX-related analysis results
        var fcxResult = scanResult.AnalysisResults.FirstOrDefault(r => r.AnalyzerName == "FCX Analyzer");
        var modConflictResult =
            scanResult.AnalysisResults.FirstOrDefault(r => r.AnalyzerName == "Mod Conflict Analyzer");
        ...
        AnalysisResult? versionResult)
    {
        ...
        if (issues.Any())
        {
            ...
            foreach (var issue in issues) report.Add($"  • {issue}\n");
            report.Add("\n");
            report.Add(
                "* For detailed FCX documentation and solutions, see: https://github.com/evildarkarchon/Scanner111/wiki/FCX-Mode *\n");
            report.Add("\n");
        }
    }
