...
public class VersionAnalyzer : IAnalyzer
{
    ...
    public async Task<AnalysisResult> AnalyzeAsync(CrashLog crashLog, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var detectedVersion = await GameVersionDetection.DetectGameVersionAsync(gameExePath, cancellationToken)
            ...
            if (IsVersionDowngrade(detectedVersion.Version, latestVersion))
            {
                ...
                reportLines.Add(
                    $"Current version ({detectedVersion.Version}) is older than the latest version ({latestVersion})\n");
            ...
            if (!File.Exists(xsePath))
            {
                ...
                reportLines.Add(
                    $"Install {xseAcronym} from https://www.nexusmods.com/{(gameType == GameType.Fallout4 ? "fallout4/mods/42147" : "skyrimspecialedition/mods/30379")}\n");
                reportLines.Add("\n");
            }
            else if (!string.IsNullOrEmpty(detectedVersion.RequiredF4seVersion))
            {
                ...
                reportLines.Add(
                    $"Game version {detectedVersion.Version} requires {xseAcronym} version {detectedVersion.RequiredF4seVersion} or higher\n");
            ...
            if (detectedVersion.Notes != null && detectedVersion.Notes.Length > 0)
            {
                ...
                foreach (var note in detectedVersion.Notes) reportLines.Add($"• {note}\n");
                reportLines.Add("\n");
            }
