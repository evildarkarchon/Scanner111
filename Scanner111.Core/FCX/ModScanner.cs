...
namespace Scanner111.Core.FCX
{
    public class ModScanner : IModScanner
    {
        ...
        private async Task InitializeAsync()
        {
            ...
            var yamlData = _yamlSettings.LoadYaml<ClassicFallout4Yaml>(yamlFile);
            
            if (yamlData != null)
            {
                _xseAcronym = yamlData.GameInfo?.XseAcronym ?? "XSE";
                
                // Load XSE script files
                if (yamlData.XseHashedScripts != null)
                {
                    foreach (var script in yamlData.XseHashedScripts.Keys)
                    {
        ...
        private async Task AnalyzeUnpackedFilesAsync(string modPath, ModScanResult result, IProgress<string>? progress, CancellationToken ct)
        {
            ...
            await foreach (var fileInfo in EnumerateFilesAsync(modPath, "*.*", ct).ConfigureAwait(false))
            {
                ...
                if (!processedDirs.Contains(dirPath) && dirPath.Contains("animationfiledata", StringComparison.OrdinalIgnoreCase))
                {
        ...
        private async Task AnalyzeArchivedFilesAsync(string modPath, string bsarchPath, ModScanResult result, IProgress<string>? progress, CancellationToken ct)
        {
            var archiveCount = 0;
        ...
        private async Task AnalyzeTextureArchive(string archivePath, string fileName, string bsarchPath, ModScanResult result, CancellationToken ct)
        {
            try
            {
        ...
        private async Task AnalyzeGeneralArchive(string archivePath, string fileName, string bsarchPath, ModScanResult result, CancellationToken ct)
        {
            try
            {
                var process = new Process
