...
namespace Scanner111.Core.FCX
{
    public class ModCompatibilityService : IModCompatibilityService
    {
        ...
        public async Task<ModCompatibilityInfo?> GetCompatibilityInfoAsync(string modName, GameType gameType, string gameVersion)
        {
            ...
            try
            {
                ...
                foreach (var req in requirements.Children.OfType<YamlMappingNode>())
                {
                    ...
                    {
                        var minVersion = req.Children.TryGetValue("min_version", out var minNode) ? (minNode as YamlScalarNode)?.Value : null;
                        var maxVersion = req.Children.TryGetValue("max_version", out var maxNode) ? (maxNode as YamlScalarNode)?.Value : null;
                        var notes = req.Children.TryGetValue("notes", out var notesNode) ? (notesNode as YamlScalarNode)?.Value : null;
                        ...
                        return new ModCompatibilityInfo
                        {
                            ...
                            RecommendedAction = !isCompatible ? GetRecommendedAction(gameVersion, minVersion, maxVersion) : null
                        };
                    }
                }
            }
        ...
        public async Task<List<ModCompatibilityIssue>> GetKnownIssuesAsync(GameType gameType, string gameVersion)
        {
            ...
            try
            {
                ...
                {
                    var affectedVersionsNode = issue.Children.TryGetValue("affected_versions", out var affNode) ? affNode as YamlSequenceNode : null;
                    ...
                    if (affectedVersions.Contains("all") || affectedVersions.Contains(gameVersion))
                    {
                        ...
                        {
                            ModName = issue.Children.TryGetValue("mod", out var modNode) ? (modNode as YamlScalarNode)?.Value ?? "" : "",
                            AffectedVersions = affectedVersions,
                            Issue = issue.Children.TryGetValue("issue", out var issueNode) ? (issueNode as YamlScalarNode)?.Value ?? "" : "",
                            Solution = issue.Children.TryGetValue("solution", out var solNode) ? (solNode as YamlScalarNode)?.Value : null
                        });
                    }
                }
            }
        ...
        public async Task<List<XsePluginRequirement>> GetXseRequirementsAsync(GameType gameType, string gameVersion)
        {
            ...
            try
            {
                ...
                {
                    var gameVersionsNode = plugin.Children.TryGetValue("game_versions", out var gvNode) ? gvNode as YamlSequenceNode : null;
                    ...
                    {
                        PluginName = plugin.Children.TryGetValue("plugin", out var plNode) ? (plNode as YamlScalarNode)?.Value ?? "" : "",
                        RequiredXseVersion = plugin.Children.TryGetValue(gameType == GameType.Fallout4 ? "required_f4se" : "required_skse", out var xseNode) ? (xseNode as YamlScalarNode)?.Value ?? "" : "",
            ...
            {
                _logger.LogError(ex, "Error getting XSE requirements for {GameType} version {Version}", gameType, gameVersion);
            }
        ...
        private string GetRecommendedAction(string gameVersion, string? minVersion, string? maxVersion)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(maxVersion) && maxVersion != "null")
                {
                    var max = Version.Parse(maxVersion);
                    if (current > max)
                        return $"This mod is not compatible with game versions newer than {maxVersion}. Check for mod updates or use an older game version.";
                }
            }
            catch
            {
