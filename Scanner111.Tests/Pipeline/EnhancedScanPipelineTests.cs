...
public class EnhancedScanPipelineTests : IDisposable
{
    ...
    /// <summary>
    ///     Validates that the <see cref="EnhancedScanPipeline.ProcessSingleAsync" /> method successfully processes a single
    ///     log file.
    /// </summary>
    /// <remarks>
    ///     This test ensures that the method correctly processes the specified log file and returns a valid
    ///     <see cref="ScanResult" /> object.
    ///     It validates that the log path in the result matches the input path, the processing status is set to Completed,
    ///     all analysis results are present, and the crash log is not null. Additionally, it verifies the processing time is
    ///     greater than zero.
    /// </remarks>
    /// <returns>
    ///     An asynchronous task that represents the test execution. The task will succeed if the method processes the file as
    ///     expected
    ...
    ///     asserting that results are properly generated and that the concurrency limit impacts the processing duration as
    ///     expected.
    ///     It verifies that the pipeline handles concurrency constraints and produces a complete set of results, whether
    ///     successful
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation of processing files,
    ///     including monitoring and canceling the operation when specific conditions are met.
    /// </returns>
    ...
    public async Task ProcessSingleAsync_UsesAnalyzerPriority()
    {
        ...
        executionOrder.Should()
            .BeEquivalentTo(new[] { "High", "Medium", "Low" }, options => options.WithStrictOrdering());
    }
...
";
    ...
    private class TestMessageHandler : IMessageHandler
    {
        ...
        /// <param name="target">
        ///     Specifies the target(s) where the message will be displayed. The default value is
        ...
        /// <param name="target">
        ///     Indicates the target audience or destination for the message. Default is
        ...
        ///     An integer specifying the total number of items to process, allowing the progress handler to
        ///     calculate completion percentage.
        /// </param>
        /// <returns>
        ///     A progress handler object implementing <see cref="IProgress{ProgressInfo}" />, which can be used to report
        ///     progress updates.
        /// </returns>
        ...
        /// <returns>
        ///     An implementation of <see cref="IProgressContext" />, providing mechanisms for reporting progress information
    ...
    /// <summary>
    ///     A test implementation of the <see cref="IYamlSettingsProvider" /> interface, primarily used for unit testing
    ///     purposes.
    /// </summary>
    /// <remarks>
