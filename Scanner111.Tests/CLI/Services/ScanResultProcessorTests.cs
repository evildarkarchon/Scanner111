...
public class ScanResultProcessorTests : IDisposable
{
    ...
    {
        // Arrange
        var result = CreateScanResult(true);
        ...
        // Act
        await _scanResultProcessor.ProcessScanResultAsync(result, options, _reportWriterMock.Object, xseCopiedFiles,
            settings);

        // Assert
        _reportWriterMock.Verify(
            x => x.WriteReportAsync(It.IsAny<ScanResult>(), It.IsAny<string>(), It.IsAny<CancellationToken>()),
            Times.Never);
        _unsolvedLogsMoverMock.Verify(
            x => x.MoveUnsolvedLogAsync(It.IsAny<string>(), It.IsAny<ApplicationSettings>(),
    ...
    public async Task ProcessScanResultAsync_WithAutoSave_WritesReport()
    {
        ...
        var result = CreateScanResult(true, logPath);
        ...
        _reportWriterMock.Setup(x =>
                x.WriteReportAsync(It.IsAny<ScanResult>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
    ...
    public async Task ProcessScanResultAsync_WithFailedScan_MovesUnsolvedLog()
    {
        ...
        _unsolvedLogsMoverMock.Setup(x =>
                x.MoveUnsolvedLogAsync(It.IsAny<string>(), It.IsAny<ApplicationSettings>(),
        ...
        // Assert
        _unsolvedLogsMoverMock.Verify(x => x.MoveUnsolvedLogAsync(logPath, settings, It.IsAny<CancellationToken>()),
            Times.Once);
    }
