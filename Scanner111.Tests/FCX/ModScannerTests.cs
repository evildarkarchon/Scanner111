...
public class ModScannerTests : IDisposable
{
    ...
    public async Task ScanAllModsAsync_WithValidPath_ScansUnpackedAndArchivedMods()
    {
        ...
        progressMessages.Should()
            .Contain("Scanning unpacked mod files...", "unpacked scan progress should be reported");
        progressMessages.Should()
            .Contain("Scanning archived mod files...", "archived scan progress should be reported");
    }
    ...
    public async Task ScanUnpackedModsAsync_ValidatesInvalidDdsDimensions()
    {
        ...
        dimensionIssue.Description.Should()
            .Contain("not divisible by 2", "description should mention divisibility issue");
    ...
    public async Task ScanUnpackedModsAsync_DetectsAnimationData()
    {
        ...
        animIssue.Description.Should()
            .Contain("custom animation file data", "description should mention animation data");
    }
    ...
    public async Task ScanUnpackedModsAsync_DetectsPrevisFiles()
    {
        ...
        previsIssue.Description.Should()
            .Contain("precombine/previs files", "description should mention precombine/previs");
    }
    ...
    public async Task ScanUnpackedModsAsync_HandlesProgressReporting()
    {
        ...
        for (var i = 0; i < 101; i++) CreateTestFile(Path.Combine(modPath, $"file{i}.txt"), "content");
        ...
        // Assert
        progressMessages.Should().Contain(msg => msg.Contains("Analyzed") && msg.Contains("files"),
            "progress should report analyzed files");
    }
    ...
    public async Task ScanArchivedModsAsync_SkipsPrpMainArchive()
    {
        ...
        result.Issues.Count(i => i.Type == ModIssueType.ArchiveFormatIncorrect).Should()
            .Be(1, "only regular.ba2 should have format issue");
    }
    ...
    public async Task ScanAllModsAsync_HandlesIoCancellation()
    {
        ...
        for (var i = 0; i < 1000; i++) CreateTestFile(Path.Combine(modPath, $"file{i}.txt"), "content");
        ...
        await action.Should()
            .ThrowAsync<OperationCanceledException>("scan should be cancelled when token is cancelled");
    }
    ...
    public async Task CleanupModFilesAsync_RemovesDocumentationFiles()
    {
        ...
        result.CleanedFiles.Should()
            .NotContain(f => f.Contains("important.txt"), "important.txt should not be cleaned");
    }
