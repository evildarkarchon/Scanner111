...
public class ModConflictAnalyzerTests
{
    ...
    public async Task AnalyzeAsync_WithTextureDimensionIssues_ReportsCorrectly()
    {
        ...
        genericResult.ReportText.Should().Contain("⚠️ DDS DIMENSIONS ARE NOT DIVISIBLE BY 2 ⚠️",
            "invalid texture dimensions should be reported");
        genericResult.ReportText.Should().Contain("Data\\Textures\\test.dds (1023x1023)",
            "the specific file and dimensions should be included");
    ...
    public async Task AnalyzeAsync_WithIncorrectTextureFormat_ReportsIssues()
    {
        ...
        genericResult.ReportText.Should().Contain("❓ TEXTURE FILES HAVE INCORRECT FORMAT, SHOULD BE DDS ❓",
            "format warning should be displayed");
        genericResult.ReportText.Should()
            .Contain("Data\\Textures\\badformat.png", "the problematic file should be listed");
    }
    ...
    public async Task AnalyzeAsync_WithXseScriptConflicts_ReportsWarnings()
    {
        ...
        genericResult.ReportText.Should().Contain("⚠️ MODS CONTAIN COPIES OF *F4SE* SCRIPT FILES ⚠️",
            "F4SE script warning should be displayed");
        genericResult.ReportText.Should()
            .Contain("Data\\F4SE\\Plugins\\test.dll", "the conflicting file should be listed");
    }
    ...
    public async Task AnalyzeAsync_WithPrevisFiles_ReportsRecommendations()
    {
        ...
        genericResult.ReportText.Should()
            .Contain("🔧 PREVIS FILES DETECTED 🔧", "previs detection header should be displayed");
        genericResult.ReportText.Should().Contain("should load after the PRP.esp plugin",
            "load order recommendation should be included");
        genericResult.ReportText.Should()
            .Contain("Data\\Meshes\\Precombined\\test.nif", "the previs file should be listed");
    }
    ...
    public async Task AnalyzeAsync_WithMultipleIssueTypes_CategorizesProperly()
    {
        ...
        genericResult.ReportText.Should()
            .Contain("❓ SOUND FILES HAVE INCORRECT FORMAT", "sound format issues should be reported");
        genericResult.ReportText.Should()
            .Contain("💀 BROKEN ANIMATION DATA FILES 💀", "animation data issues should be reported");
    }
    ...
    public async Task AnalyzeAsync_WithSkyrimGame_UsesCorrectAcronym()
    {
        ...
        // Assert
        var genericResult = (GenericAnalysisResult)result;
        genericResult.ReportText.Should().Contain("⚠️ MODS CONTAIN COPIES OF *SKSE* SCRIPT FILES ⚠️",
            "SKSE should be used for Skyrim instead of F4SE");
    }
    ...
    public async Task AnalyzeAsync_WithCleanupFiles_ReportsAsWarning()
    {
        ...
        genericResult.ReportText.Should()
            .Contain("🗑️ DETECTED UNINTENDED FILES 🗑️", "cleanup file warning should be displayed");
