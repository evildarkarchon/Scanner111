...
public class FcxReportGenerationTests : IDisposable
{
    ...
    {
        // Arrange
        var gameDir = CreateMockGameInstallation(true, true);
    ...
    {
        // Arrange
        var gameDir = CreateMockGameInstallation(false, true);
        _settingsService.Settings.FcxMode = true;
    ...
    {
        // Arrange
        var gameDir = CreateMockGameInstallation(true, false);
        _settingsService.Settings.FcxMode = true;
    ...
    {
        // Arrange
        var gameDir = CreateMockGameInstallation(false, false);
        _settingsService.Settings.FcxMode = true;
    ...
    private string GenerateFcxReport(FcxScanResult? result)
    {
        ...
        if (result.FileChecks.Any())
        {
            ...
            foreach (var check in result.FileChecks)
            {
                ...
                if (!string.IsNullOrEmpty(check.FilePath)) sb.AppendLine($"      Path: {check.FilePath}");
            }
        ...
        if (result.VersionWarnings.Any())
        {
            ...
            foreach (var warning in result.VersionWarnings) sb.AppendLine($"  - {warning}");
            sb.AppendLine();
        }
        ...
        if (result.RecommendedFixes.Any())
        {
            ...
            foreach (var fix in result.RecommendedFixes) sb.AppendLine($"  • {fix}");
            sb.AppendLine();
        }

        // Use the actual report text if available
        if (!string.IsNullOrEmpty(result.ReportText)) sb.AppendLine(result.ReportText);
