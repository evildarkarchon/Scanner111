...
public class GameScannerServiceTests
{
    ...
    public async Task ScanGameAsync_AllScannersSucceed_ReturnsCompleteResults()
    {
        ...
        _mockMessageHandler.Verify(
            x => x.ShowInfo(It.Is<string>(s => s == "Starting comprehensive game scan..."), It.IsAny<MessageTarget>()),
            Times.Once);
    ...
    public async Task ScanGameAsync_OneScannerFails_ContinuesAndReportsError()
    {
        ...
        _mockXsePluginValidator.Setup(x => x.ValidateAsync())
            .ThrowsAsync(new InvalidOperationException("XSE validation failed"));
        _mockModIniScanner.Setup(x => x.ScanAsync()).ReturnsAsync(modIniResult);
        _mockWryeBashChecker.Setup(x => x.AnalyzeAsync()).ReturnsAsync(wryeBashResult);
    ...
    public async Task ScanGameAsync_WithCancellation_ThrowsOperationCancelledException()
    {
        ...
        _mockMessageHandler.Verify(
            x => x.ShowWarning(It.Is<string>(s => s == "Game scan cancelled"), It.IsAny<MessageTarget>()), Times.Once);
    }
    ...
    public async Task CheckCrashGenAsync_Success_ReturnsResult()
    {
        ...
        _mockMessageHandler.Verify(
            x => x.ShowInfo(It.Is<string>(s => s == "Checking Crash Generator configuration..."),
                It.IsAny<MessageTarget>()), Times.Once);
        _mockMessageHandler.Verify(
            x => x.ShowSuccess(It.Is<string>(s => s == "Crash Generator check completed"), It.IsAny<MessageTarget>()),
            Times.Once);
    }
    ...
    public async Task CheckCrashGenAsync_Exception_ReturnsErrorMessage()
    {
        ...
        _mockMessageHandler.Verify(
            x => x.ShowError(It.Is<string>(s => s.Contains(errorMessage)), It.IsAny<MessageTarget>()), Times.Once);
    }
    ...
    public async Task ValidateXsePluginsAsync_Success_ReturnsResult()
    {
        ...
        result.Should().Be(expectedResult);
        _mockMessageHandler.Verify(
            x => x.ShowInfo(It.Is<string>(s => s == "Validating XSE plugins..."), It.IsAny<MessageTarget>()),
            Times.Once);
        _mockMessageHandler.Verify(
            x => x.ShowSuccess(It.Is<string>(s => s == "XSE plugin validation completed"), It.IsAny<MessageTarget>()),
            Times.Once);
    }
    ...
    public async Task ScanModInisAsync_Success_ReturnsResult()
    {
        ...
        result.Should().Be(expectedResult);
        _mockMessageHandler.Verify(
            x => x.ShowInfo(It.Is<string>(s => s == "Scanning mod INI files..."), It.IsAny<MessageTarget>()),
            Times.Once);
        _mockMessageHandler.Verify(
            x => x.ShowSuccess(It.Is<string>(s => s == "Mod INI scan completed"), It.IsAny<MessageTarget>()),
            Times.Once);
    }
    ...
    public async Task CheckWryeBashAsync_Success_ReturnsResult()
    {
        ...
        result.Should().Be(expectedResult);
        _mockMessageHandler.Verify(
            x => x.ShowInfo(It.Is<string>(s => s == "Analyzing Wrye Bash report..."), It.IsAny<MessageTarget>()),
            Times.Once);
        _mockMessageHandler.Verify(
            x => x.ShowSuccess(It.Is<string>(s => s == "Wrye Bash analysis completed"), It.IsAny<MessageTarget>()),
            Times.Once);
    }
