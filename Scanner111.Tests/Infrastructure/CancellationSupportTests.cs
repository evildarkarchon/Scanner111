...
public class CancellationSupportTests : IDisposable
{
    ...
    /// <summary>
    ///     Validates that the <see cref="EnhancedCancellationTokenSource" /> correctly cancels its token when the
    ///     <see cref="EnhancedCancellationTokenSource.Cancel" /> method is invoked.
    /// </summary>
    /// <remarks>
    ///     This test ensures that the cancellation state of both the <see cref="EnhancedCancellationTokenSource" /> and its
    ///     associated <see cref="CancellationToken" />
    ///     reflect the expected behavior when cancellation is requested. It verifies that both
    ///     <see cref="EnhancedCancellationTokenSource.IsCancellationRequested" />
    ///     and <see cref="CancellationToken.IsCancellationRequested" /> return true after invoking the
    ///     <see cref="EnhancedCancellationTokenSource.Cancel" /> method.
    /// </remarks>
    [Fact]
    ...
    ///     the token is cancelled automatically once the timeout duration has expired. A sleep period longer than the timeout
    ...
    ///     after the specified delay has elapsed, verifying the correct behavior of the
    ///     <see cref="EnhancedCancellationTokenSource.IsCancellationRequested" /> property.
    /// </remarks>
    [Fact]
    ...
    ///     throws an <see cref="OperationCanceledException" /> when the associated <see cref="CancellationToken" /> is
    ///     cancelled.
    /// </summary>
    /// <remarks>
    ...
    ///     It verifies the functionality by asserting initial cancellation state and observing state changes upon
    ///     cancellation.
    /// </remarks>
    [Fact]
    public void CancellationHelper_CreateLinkedTokenSource_Works()
    {
        ...
        // Cancel one source
        cts1.Cancel();
        linked.Token.IsCancellationRequested.Should()
            .BeTrue("linked token should be cancelled when any source is cancelled");
    }
    ...
    /// </summary>
    /// <remarks>
    ///     This test ensures that the combined cancellation token generated by
    ///     <see cref="CancellationHelper.CreateCombinedToken" />
    ...
    public void CancellationHelper_CreateCombinedToken_CombinesUserAndTimeout()
    {
        ...
        // Act & Assert
        combinedSource.Token.IsCancellationRequested.Should()
            .BeFalse("combined token should not be cancelled initially");

        // Cancel user token
        userSource.Cancel();
        combinedSource.Token.IsCancellationRequested.Should()
            .BeTrue("combined token should be cancelled when user token is cancelled");
    }
    ...
    /// </summary>
    /// <returns>
    ///     Ensures that the task returned by the
    ///     <see cref="CancellationTokenExtensions.WaitForCancellationAsync(CancellationToken)" /> method
    ...
    ///     <see cref="CancellationTokenExtensions.WithCancellation{T}(Task{T}, CancellationToken)" />
    ...
    ///     including confirming the correct propagation of progress values to the wrapped <see cref="IProgress{T}" />
    ///     implementation
