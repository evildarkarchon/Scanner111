...
public class MessageHandlerTests
{
    ...
    /// <summary>
    ///     Validates that the `MsgInfo` method of the `MessageHandler` class correctly invokes the handler for informational
    ///     messages.
    /// </summary>
    /// <remarks>
    ///     This test ensures that calling the `MsgInfo` method registers the provided message in the handler's `InfoMessages`
    ///     collection
    ///     and sets the `LastInfoTarget` property to the appropriate `MessageTarget` value. It verifies the correct
    ///     functioning of
    ...
    public void MessageHandler_MsgWarning_CallsHandler()
    {
        ...
        _testHandler.WarningMessages.Should()
            .Contain("Test warning message", "because warning message was sent to handler");
    }
    ...
    /// </summary>
    /// <remarks>
    ///     This test ensures that the `MsgProgress` method passes the provided title and total item count to the handler and
    ///     that
    ///     a valid progress object is returned. The behavior being tested is critical for progress reporting functionality
    ///     within
    ///     the application.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    ///     Confirms that progress updates are tracked and reported correctly by the `ProgressContext` created via the
    ///     `MessageHandler`.
    ///     The test verifies that the progress updates are recorded in the correct order with accurate values for `Current`,
    ///     `Message`,
    ...
    public void MessageHandler_ProgressContext_ReportsProgressCorrectly()
    {
        ...
        testProgressContext.Reports[0].Message.Should()
            .Be("Processing item 10", "because first update message was provided");

        testProgressContext.Reports[1].Current.Should().Be(25, "because second update set current to 25");
        testProgressContext.Reports[1].Message.Should()
            .Be("Processing item 25", "because second update message was provided");
    ...
    /// <summary>
    ///     Validates that the MessageHandler correctly tracks and reports progress updates using the provided progress
    ///     reporter.
    /// </summary>
    /// <remarks>
    ...
    public void MessageHandler_Progress_ReportsProgressCorrectly()
    {
        ...
        testProgress.Reports[1].Message.Should()
            .Be("Three quarters done", "because second report message was provided");
    }
    ...
    /// </summary>
    /// <remarks>
    ///     Ensures the correct initialization and retrieval of all `MessageTarget` enumeration values: `All`, `GuiOnly`, and
    ///     `CliOnly`.
    ///     This test confirms the integrity and usability of the `MessageTarget` enum for its intended purposes within the
    ///     application.
    /// </remarks>
    /// <exception cref="Xunit.Sdk.EqualException">
...
public class TestMessageHandler : IMessageHandler
{
    ...
    /// <param name="target">
    ///     Optional target type that specifies where the message should be logged. Defaults to
    ///     <c>MessageTarget.All</c>.
    /// </param>
    /// <remarks>
    ...
    /// <remarks>
    ///     The method handles different message types such as Info, Warning, and Error by categorizing them into corresponding
    ///     collections.
    ...
    /// <summary>
    ///     Creates a new progress context with the specified title and total number of items to track progress during an
    ///     operation.
    /// </summary>
...
public class TestProgressContext : IProgressContext
{
    ...
    /// </summary>
    /// <param name="current">
    ///     The current progress value, representing the number of completed items or the current step in the
    ///     operation.
    /// </param>
    ...
    ///     and no further updates are expected. Ensure all necessary progress updates have been reported before invoking this
    ///     method.
    /// </remarks>
