...
public class ScanLogServiceTests
{
    ...
    public async Task ScanLogFileAsync_FormatsBeforeParsing()
    {
        ...
        _mockFormattingService
            .Setup(x => x.ReformatCrashLogsAsync(It.Is<IEnumerable<string>>(f => f.Contains(testFile)),
        ...
        // Assert
        _mockFormattingService.Verify(
            x => x.ReformatCrashLogsAsync(It.Is<IEnumerable<string>>(f => f.Contains(testFile)),
                _appSettings.SimplifyRemoveStrings),
            Times.Once
        );
