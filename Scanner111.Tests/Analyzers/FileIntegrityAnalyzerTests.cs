...
public class FileIntegrityAnalyzerTests : IDisposable
{
    ...
    public async Task AnalyzeAsync_WithF4SEInstalled_DetectsF4SE()
    {
        ...
        var gameDir = CreateTempGameDirectory(includeF4SE: true);
        var crashLog = new CrashLog
        {
            FilePath = "test.log",
            GamePath = gameDir
    ...
    public async Task AnalyzeAsync_WithMissingF4SE_AddsRecommendation()
    {
        ...
        var gameDir = CreateTempGameDirectory(includeF4SE: false);
        var crashLog = new CrashLog
        {
            FilePath = "test.log",
            GamePath = gameDir
    ...
    public async Task AnalyzeAsync_DeterminesCorrectOverallStatus()
    {
        ...
        var gameDir = CreateTempGameDirectory(includeF4SE: true, includeCoreMods: true);
        var crashLog = new CrashLog
        {
            FilePath = "test.log",
            GamePath = gameDir
...
public class TestHashValidationService : IHashValidationService
{
    ...
    {
        if (_knownHashes.TryGetValue(filePath, out var hash))
        {
